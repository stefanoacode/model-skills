...
## Formula-First Rule (Building or Completing Models)

**Every calculated cell must contain a live Excel formula, never a hardcoded value.**

When generating a model from scratch using Python (openpyxl/xlsxwriter), this means:
- **Input cells** (historical actuals, assumption drivers): hardcoded values with blue font
- **All other cells**: must be Excel formulas that reference inputs or other cells
- The model must be **fully dynamic** — changing one assumption should cascade through IS → BS → CF

**Examples:**
| Cell | Wrong (hardcoded) | Right (formula) |
|------|-------------------|-----------------|
| Gross Profit | `9410` | `=C6-C7` (Revenue - COGS) |
| COGS (projected) | `53800` | `=IS!C6*Assumptions!C8` (Revenue × CoS%) |
| Ending Cash (CF) | `15200` | `=C25+C22` (Beginning Cash + Net Change) |
| BS Check | `0` | `=C10-C20-C25` (Assets - Liabilities - Equity) |
...
# Right — live formula
ws['C8'] = '=C6-C7'  # Gross Profit = Revenue - COGS
```

**Cross-tab references must use sheet names:**
```python
ws_cf['C5'] = "=IS!C28"       # NI from IS to CF
ws_bs['C6'] = "=CF!C22"       # Ending Cash from CF to BS
ws_checks['C5'] = "=BS!C10-BS!C20-BS!C25"  # BS balance check
```

Only hardcode: historical actuals sourced from filings, and assumption driver inputs. Everything else is a formula.
